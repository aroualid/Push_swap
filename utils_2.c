...
/*   Updated: 2024/03/15 14:13:14 by aroualid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	right_pos(t_stack *b, int nb)
{
	...
	i = 1;
	j = 0;
	if (b->data && nb > b->data[0] && nb < b->data[b->size - 1])
		return (0);
	if ((b->data) && (nb > b->data[maxus(b)] || nb < b->data[minus(b)]))
		return (maxus(b));
	while ((b->data) && (nb > b->data[j] || nb < b->data[j + 1]))
	{
		i++;
		j++;
	}
